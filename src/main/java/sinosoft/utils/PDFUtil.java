...
public class PDFUtil {
	 ...
	 * @param header
	 */
	public void CreatValuationTable(String [] headorder,List list,String productName,String time,String unitNet,String opertor,String create_operator) {
	 try{
		// 创建一个表格
		...
		for(int k = 0 ; k < page ; k++){
			...
			// 添加备注,靠左，不显示边框
			headTable.addCell(createCell("基金资产估值表", headfont, Element.ALIGN_CENTER, overhead.length,false));
			if(productName.contains("@@")){
				productName = productName.split("@@")[0];
			}
			headTable.addCell(createCell(productName+"_专用表", keyfont, Element.ALIGN_CENTER, overhead.length,false));
			...
			if(null != list && list.size() > 0){
				...
				if(i<list.size()){	
					...
					String fkmbm = (String) map.get("fkmbm");
	 ...
	 * @return  //excel所在的路径
	 */
	public String CreatValuationTables(String [] headorder,List list,String saveFilePathAndName,String productName,String time,String unitNet, String opertor,String create_operator) throws Exception{
		File file = new File(saveFilePathAndName);
			file.delete();
		if(!file.exists()){
			file.createNewFile();
			CreatePdf(file);
			CreatValuationTable(headorder,list,productName,time,unitNet,opertor,create_operator); 
			return saveFilePathAndName;
		}else{
			return "";
		}
