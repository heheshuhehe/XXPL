 ...
 */
@Controller
public class IdCardVerificationController {
	...
	public String idCardVerification(HttpServletRequest request,HttpServletResponse response){
		PageData pd1 = new PageData();
		String idStr=request.getParameter("idCard");//身份证号
		String verResult=IdentityCardVerification(idStr);//验证身份证号
		if(verResult.isEmpty()){
			//验证成功
			pd1.put("result_code", "ok");
			pd1.put("err_message", "");
			pd1.put("paramString", "");
		}else{
			//验证失败
			pd1.put("result_code", "error");
			pd1.put("err_message", verResult);
			pd1.put("paramString", idStr);
		}
		return JSONObject.fromObject(pd1).toString();
	}
    ...
    public static void main(String[] args) {
    	IdentityCardVerification("31022619801204454X");
	}
}
