...
public class ExcelOperationUtil {
	 ...
	 *
	 * <br/>
	 * 建议用于 不同book间复制，同时复制数据和样式<br/>
	 * eg: copySheet(srcSheet, desSheet, mapping)
	 *
	 * @param mapping
	 *            不同文件间复制时，如果要复制样式，必传，否则不复制样式
	 */
	 ...
	 *            不同book中复制样式时，必传
	 */
	public static void copySheet(Sheet srcSheet, Sheet desSheet, boolean copyValueFlag, boolean copyStyleFlag,
	 ...
	 * 复制行
	 */
	public static void copyRow(Row srcRow, Row desRow, boolean copyValueFlag, boolean copyStyleFlag,
			StyleMapping mapping) {
	 ...
	 *            控制是否复制样式
	 * @param mapping
	 *            不同文件间复制时，如果需要连带样式复制，必传，否则不复制样式
	 */
	...
	public static StyleMapping copyCellStyle(Workbook srcBook, Workbook desBook) {
		...
		logger.debug(String.format("src中style number:{%s}, des中style number:{%s}", srcBook.getNumCellStyles(),
				desBook.getNumCellStyles()));
