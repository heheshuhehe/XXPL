...
function createwbdata(){
	...
	var p_zt =$("#p_zt").combobox('getValue');
 	...
 	"&systype=2"+"&oper="+oper+"&p_zt="+p_zt+"&datatype="+encodeURIComponent($("#datatype0").combobox('getValue'));
...
function adddata2(oper){
	 ...
	 $('#oper2').val(oper);
	 if(oper=='s'){
		 var params="wbzth="+$("#wbzth2").combobox('getValue')+"&filetype="+$("#filetype2").combobox('getValue')+"&datatype="+encodeURIComponent($("#datatype").combobox('getValue'));
...
function saveInfonew2(){
	...
	$.ajax({
		url:'<%=cp%>/guzhidz/queryqsdataconfig',
		type:'post',
		data:{ider:ider,filetype:filetype,jyjg:jyjg,wbzth:wbzth,datasrc:datasrc,sender:sender,subjecter:subjecter,contenter:contenter,attacher:attacher,filepath:filepath,datefloat:datefloat,datefloattype:datefloattype,oper:oper,systype:2,tzqd:tzqd,datatype:datatype},
		dataType: "json",
		success: function(data){
			if(data[0].msg=="success"){  
				alert("保存成功");
...
<div id="tabsidk" class="easyui-tabs" style="">
	<div title="数据处理情况查询" style="padding: 10px">
		<table>
				...
				</select>	估值频率：<select class="easyui-combobox"	name="datatype0" id="datatype0" style="width: 100px;" > 
				...
				<input style="display: none;"  type="checkbox" name="ismx" id="ismx" >
					...
					<a  style="display: none" href="javascript:void(0);" class="easyui-linkbutton"data-options="iconCls:'icon-search'" onclick="createwbdata2('2','do')">外包数据处理</a>
					<a href="javascript:void(0);" class="easyui-linkbutton"data-options="iconCls:'icon-search'" onclick="createwbdata2('2','redo')">数据处理</a>
					</td>		
				</tr>
		</table>
...
<div id="dialog2" class="easyui-dialog" closed="true" title="添加/修改"
			...
			<br /> <br />
					...
					关键字：<input class="easyui-textbox"	name="tzqd" id="tzqd" style="width: 140px;" > 
						...
						估值频率：<select class="easyui-combobox"	name="datatype_dia2" id="datatype_dia2" style="width: 100px;" > 
						<option value="T+0">T+0</option>
						<option value="T+1">T+1</option>
