...
function createwbdata(){
	var ztmc=$("#wbzth").combobox('getText')
 	var params="wbdate="+$("#wbdate").combobox('getValue')+"&valdate="+$("#valdate").combobox('getValue')+"&ztmc="+ztmc+"&status="+$("#status").combobox('getValue')+"&op=s";
	 
	params=params+"&valdate2="+$("#valdate2").combobox('getValue');
	$('#wbdg').datagrid({method:'get',url:'<%=cp%>/guzhidz/automailquerylog4glr?'+params}).datagrid('clientPaging');
...
function sendmail(){
	...
	$.ajax({
		url:'<%=cp%>/guzhidz/automailquerylog4glr?op=send',
		type:'post',
		data:{date:date},
		dataType: "json",
	...
	<div id="p" class="easyui-panel" title="数据字典" style="width:95%;height:150px;padding:10px;"
		...
		<table>
				...
				<span>发送日期</span>
					...
					<a href="javascript:void(0);" class="easyui-linkbutton"data-options="iconCls:'icon-search'"  " onclick="sendmail()">发送</a>
					<a href="javascript:void(0);" class="easyui-linkbutton"data-options="iconCls:'icon-search'"  style="display: none;"  onclick="adddata()">发送状态重置</a>
					</td><td></td>		
				</tr>
		</table>
				...
				<thead>
					<tr>
					 <th data-options="field:'ck',checkbox:true"></th>
						...
						<th field="zipbatch_id" >批次信息</th>
						<th field="needzip" >不压缩（0）</br>压缩（1）</th>
