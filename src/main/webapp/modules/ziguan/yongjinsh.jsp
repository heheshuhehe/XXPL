      ...
      $.ajax({
   	 ...
   	 url: "<%=cp%>/ziguan/initYongJin?type="+$("#firsttype").combobox('getValues').toString(),
   	   success: function(data){
   	        ...
   	        if(type4!=null){
   	        	var columns="[[{field:'account_id',title:'账套号',width:100},{field:'fsetname',title:'基金名称'},{field:'2206fendbal_qc',title:'应付管理人报酬期初余额',align:'right',formatter:myformatter},{field:'2206fendbal_tz',title:'管理人报酬期初调整',align:'right',formatter:myformatter},{field:'22090101fendbal_qc',title:'应付管理人佣金期初金额',align:'right',formatter:myformatter},{field:'22090101fendbal_tz',title:'应付管理人佣金金额调整',align:'right',formatter:myformatter},{field:'type',title:'数据库来源',hidden:true},";
   	        	var arr = type4.split("+");
...
function Yjinitialize(){
	...
	$.ajax({
	   ...
	   success: function(data){
	        ...
	        if(type4!=null){
	        	var columns="[[{field:'account_id',title:'账套号',width:100},{field:'fsetname',title:'基金名称'},{field:'type',title:'数据库来源',hidden:true},";
   	        	var arr = type4.split("+");
  	        	for (var i = 0; i < arr.length; i++) {
   					var subarr = arr[i].split(",");
   					if(subarr.length==3){
   						...
   						var thname = subarr[2];
   						if (thname == "应付管理人报酬余额"){
   							columns+="{field:'"+field+"_qc',title:'应付管理人报酬期初余额',align:'right',formatter:myformatter},";
   							columns+="{field:'"+field+"_tz',title:'管理人报酬期初余额调整',align:'right',formatter:myformatter},";
   							for(var j=0;j<arr.length;j++){
   								var subarr1 = arr[j].split(",");
   			   					if(subarr1.length==3){
   			   						var field1 = subarr1[0]+subarr1[1];
   			   						var thname1 = subarr1[2];
   			   						if (thname1 == "管理人报酬（当月）"){
   			   							columns+="{field:'"+field1+"_jf',title:'管理人报酬（当月借方）',align:'right',formatter:myformatter},";
   			   							columns+="{field:'"+field1+"',title:'管理人报酬（当月贷方）',align:'right',formatter:myformatter},";
   			   						}
   			   					}
   							}
   							columns+="{field:'"+field+"',title:'应付管理人报酬余额',align:'right',formatter:myformatter},";
   						} else if (thname == "应付管理人佣金余额"){
   							columns+="{field:'"+field+"_qc',title:'应付管理人佣金期初余额',align:'right',formatter:myformatter},";
   							columns+="{field:'"+field+"_tz',title:'应付管理人佣金期初金额调整',align:'right',formatter:myformatter},";
   							for(var j=0;j<arr.length;j++){
   								var subarr1 = arr[j].split(",");
   			   					if(subarr1.length==3){
   			   						var field1 = subarr1[0]+subarr1[1];
   			   						var thname1 = subarr1[2];
	   			   					 if (thname1 == "管理佣金（当月）"){
	   		   							columns+="{field:'"+field1+"_jf',title:'管理佣金（当月借方））',align:'right',formatter:myformatter},";
	   		   							columns+="{field:'"+field1+"',title:'管理佣金（当月贷方）',align:'right',formatter:myformatter},";
	   		   						} 
   			   					}
   							}
   							columns+="{field:'"+field+"',title:'应付管理人佣金余额',align:'right',formatter:myformatter},";
   						}
   					}
   				}
   	        	for (var i = 0; i < arr.length; i++) {
	...
	if (t==1) {
		if(firsttype==""){
			...
			var params="dlgReleaseTime="+dlgReleaseTime+"&firsttype="+firsttype+
	   		"&first="+first+"&second="+second+"&chanzt="+chanzt+"&type=sh";
		}else {
			var params="dlgReleaseTime="+dlgReleaseTime+"&firsttype="+firsttype+
	   		"&secondtype="+secondtype+"&first="+first+"&second="+second+"&chanzt="+chanzt+"&type=sh";
		}
		}
		window.location.href='<%=cp%>/ziguan/getYongJinInfoDownSH?'+params;
		}
	if(t==2){
		if(userInfo==""){
			alert("请选择估计人员");
...
if (t==1) {
	...
	$.messager.confirm("Confirm","确认要保存？保存可能会覆盖历史内容.",function(r){
		if(r){
			$.ajax({
				url:'<%=cp%>/ziguan/savedata',
				type:'post',
				data:{dlgReleaseTime:dlgReleaseTime,firsttype:firsttype,first:first,second:second,chanzt:chanzt,secondtype:secondtype},
				dataType: "json",
				success: function(data){
					if(data.msg="success"){
						 alert("保存成功");
...
if(t==2){
	...
	$.ajax({
		url:'<%=cp%>/ziguan/savedata',
		type:'post',
		data:{dlgReleaseTime:dlgReleaseTime,user_id:userInfo,user:user},
		dataType: "json",
		success: function(data){
			if(data.msg="success"){
				 alert("保存成功");
			...
			if(secondtype==""){
				...
				$('#dg').datagrid({method:'post',
					...
					queryParams:{dlgReleaseTime:dlgReleaseTime,
						         firsttype:firsttype,chanzt:chanzt,type:'sh'
						        }
	   ...
	   <div id="tabsidk" class="easyui-tabs" style="">
         ...
         <div id="p_dlg_add" class="easyui-dialog" closed="true" title="添加配置项"
			...
			<div style="margin-bottom: 10px;">
			 ...
			 <div id="div_zth">
			  <span>账套名称:</span> 
			  <span><input class="easyui-combobox" name="p_zth_dialog" id="p_zth_dialog" data-options="editable:true"></input></span>
			  <span style="color: red;">*</span>
			 </div> 
