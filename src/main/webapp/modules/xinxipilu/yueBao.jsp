...
$(function (){
	 ...
	 $("#heduiTable").datagrid({
    	...
    	onCheck:function(rowIndex){//选中时，将唯一元素存入数组中
    		 var row = $("#heduiTable").datagrid("getRows");
    		 //alert(rowIndex+"增加的rowindex");
    		 var line = row[rowIndex].fund_code;
    	...
    	onLoadSuccess : function(){//重新加载的时候，因为用的选中的方法，所以每当重新进入方法就会又有元素加进去，所以先去除重复的元素
    		...
    		for(var i =0; i<row.length;i++){
    			var a = row[i].fund_code
    			var index =  n.indexOf(a);
    			if(index>-1){
    				//alert(1234)
    				$("#heduiTable").datagrid("selectRow",i);
    			}
    		}
    	}*/
	 });
...
function search1FundDataChkRslt(dateNO,fundCode,fundName){
	...
	var params="reportType=M&dateNO="+dateNO+"&fundCode="+fundCode+"&heDuiYiZhi=0";
	 $('#mingXiTable').datagrid({method:'post',url:"<%=cp%>/xinxipilu/search1FundDataChkRslt?"+params}).datagrid('clientPaging'); 
	 $('#monthNOMingXi').combobox('setValue',dateNO);
...
<body>
	<div id="tabsidk" class="easyui-tabs" >
		<div title="核对" style="padding: 5px">
			<div title="月报核对主页" style="padding: 5px">
				<div style="margin-top: 5px; margin-bottom: 5px;">
					<span>月份:</span>  <input class="easyui-combobox" name="monthNO"
						id="monthNO"
						data-options="
				                      valueField: 'id',
									  textField: 'value',
 ...
 -->								<!-- <th field="fund_type" 			width="80" sortable="true" align="center">基金类型</th> -->
		...
		<div title="明细" style="padding: 5px">
			...
			<!-- <a href="javascript:void(0);" onclick="	 $('#monthNOMingXi').combobox('setValue','test');">test </a> -->
				<span>月份:</span>  <input class="easyui-combobox" name="monthNOMingXi"
							id="monthNOMingXi"
							data-options="
					                      valueField: 'id',
										  textField: 'value',
